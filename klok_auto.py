        ...
        while True:
            try:
                ...
                if login_flag:
                    ...
                    logger.debug(f'{self.index}, {self.proxy}, 一轮聊天成功, 睡眠 6小时')
                    await asyncio.sleep(6*3600)
            except Exception as e:

                logger.error(f'{self.index}, {self.proxy} 任务异常: {e}, 睡眠3小时')
                await asyncio.sleep(3*3600)
...
async def main(JS_SERVER: str):
    ...
    refs = []
    with open('./reg/referral_code.txt', 'r', encoding='utf-8') as file:
        for line in file.readlines():
